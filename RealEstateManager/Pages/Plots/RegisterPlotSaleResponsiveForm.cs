...
namespace RealEstateManager.Pages.Plots
{
    public partial class RegisterPlotSaleResponsiveForm : Form
    {
        private bool _isEditMode;
        private int? _editPlotId;
        private int? _editPropertyId;

        // Edit mode constructor
        public RegisterPlotSaleResponsiveForm(
            int propertyId,
            int plotId,
            string plotNumber,
            string customerName,
        ...
        ) : this()
        {
            ...
            textBoxSaleAmount.Text = decimal.TryParse(salePrice, out var saleAmt) ? saleAmt.ToString("N2") : "0.00";
            ...
            buttonRegisterSale.Text = string.IsNullOrEmpty(customerName) && string.IsNullOrEmpty(salePrice) ? "Register Sale" : "Edit Sale";
            LoadAgents();

            bool foundPlotSale = false;
            if (_editPlotId.HasValue)
            {
        ...
        private void ButtonRegisterSale_Click(object? sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrWhiteSpace(customerEmail) && !Regex.IsMatch(customerEmail, @"^[^@\s]+@[^@\s]+\.[^@\s]+$"))
            {
            ...
            string createdBy = (!string.IsNullOrEmpty(RealEstateManager.Pages.BaseForm.LoggedInUserId)) ? RealEstateManager.Pages.BaseForm.LoggedInUserId.ToString() : Environment.UserName;
                    ...
                    using (var tran = conn.BeginTransaction())
                    {
                        try
                        {
                                ...
                                "UPDATE Plot SET Status = @Status, ModifiedBy = @ModifiedBy, ModifiedDate = @ModifiedDate WHERE Id = @PlotId", conn, tran))
                            {
                                ...
                                cmd.Parameters.AddWithValue("@PlotId", _editPlotId.Value);
                                cmd.Parameters.AddWithValue("@ModifiedBy", (!string.IsNullOrEmpty(RealEstateManager.Pages.BaseForm.LoggedInUserId)) ? RealEstateManager.Pages.BaseForm.LoggedInUserId.ToString() : Environment.UserName);
