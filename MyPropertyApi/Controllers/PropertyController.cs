    ...
    public class PropertyController : ControllerBase
    {
        ...
        public async Task<ActionResult> DeleteProperty(int id)
        {
            ...
            try
            {
                ...
                string delete = @"UPDATE Property SET IsDeleted = 1, ModifiedBy = @ModifiedBy, ModifiedDate = @ModifiedDate WHERE Id = @Id";
        ...
        public async Task<ActionResult<IEnumerable<PropertyTransactionDto>>> GetActivePropertyTransactions()
        {
        ...
        public async Task<ActionResult<PropertyTransactionDto>> GetPropertyTransactionById(int id)
        {
            string? connectionString = _config.GetConnectionString("MyPropertyDb");
            if (string.IsNullOrWhiteSpace(connectionString))
                return StatusCode(500, "Database connection string is missing.");
            ...
            try
            {
                ...
                if (await reader.ReadAsync())
                {
                    var transaction = new PropertyTransactionDto
                    {
                        TransactionId = reader["TransactionId"] == DBNull.Value ? 0 : Convert.ToInt32(reader["TransactionId"]),
                        PropertyId = reader["PropertyId"] == DBNull.Value ? 0 : Convert.ToInt32(reader["PropertyId"]),
                        TransactionDate = reader["TransactionDate"] == DBNull.Value ? DateTime.MinValue : Convert.ToDateTime(reader["TransactionDate"]),
                ...
                {
                    return NotFound($"PropertyTransaction with ID {id} not found.");
                }
            }
            catch (Exception ex)
            {
                return StatusCode(500, $"Error reading property transaction: {ex.Message}");
            }
        }
        ...
        public async Task<ActionResult> EditPropertyTransaction(int id, [FromBody] PropertyTransactionDto transaction)
        {
            string? connectionString = _config.GetConnectionString("MyPropertyDb");
            if (string.IsNullOrWhiteSpace(connectionString))
                return StatusCode(500, "Database connection string is missing.");
            ...
            try
            {
                using var conn = new SqlConnection(connectionString);
                await conn.OpenAsync();
                string update = @"UPDATE PropertyTransaction SET
                ...
                int rows = await cmd.ExecuteNonQueryAsync();
                if (rows > 0)
                    return Ok(new { Success = true, Message = "Property transaction updated successfully." });
                else
                    return NotFound("Property transaction not found or already deleted.");
            }
            catch (Exception ex)
            {
                return StatusCode(500, $"Error updating property transaction: {ex.Message}");
            }
        }
