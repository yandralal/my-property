...
{
    public partial class LandingFormResponsive : BaseForm
    {
        ...
        private void SetupPlotGrid()
        {
            ...
            dataGridViewPlots.ScrollBars = ScrollBars.Both;
            ...
            {
                Name = "CustomerName",
                HeaderText = "Customer Name" // Changed header
            });
            dataGridViewPlots.Columns.Add(new DataGridViewTextBoxColumn
            {
                Name = "CustomerPhone",
            ...
            dataGridViewPlots.SizeChanged -= DataGridViewPlots_SizeChanged;
            dataGridViewPlots.SizeChanged += DataGridViewPlots_SizeChanged;
        ...
        {
            int totalGridWidth = dataGridViewPlots.Width;
            int plotNumberWidth = 100;
            int customerPhoneWidth = 130;
            int actionColWidth = 160;
            int customerNameWidth = 180;
            var visibleCols = dataGridViewPlots.Columns.Cast<DataGridViewColumn>()
                .Where(c => c.Visible && c.Name != "Id" && c.Name != "HasSale").ToList();
            int colCount = visibleCols.Count;
            if (colCount == 0) return;

            // Calculate remaining width after fixed columns
            int variableColsCount = visibleCols.Count(c => c.Name != "PlotNumber" && c.Name != "Action" && c.Name != "CustomerName" && c.Name != "CustomerPhone");
            int remainingWidth = totalGridWidth - 55 - plotNumberWidth - actionColWidth - customerNameWidth - customerPhoneWidth;
            int dynamicColWidth = variableColsCount > 0 ? remainingWidth / variableColsCount : remainingWidth;
            int remainder = variableColsCount > 0 ? remainingWidth % variableColsCount : 0;

            foreach (var col in visibleCols)
            {
                if (col.Name == "PlotNumber")
                {
                    col.Width = plotNumberWidth;
                }
                else if (col.Name == "CustomerName")
                {
                    col.Width = customerNameWidth;
                }
                else if (col.Name == "CustomerPhone")
                {
                    col.Width = customerPhoneWidth;
                }
                ...
                {
                    int idx = visibleCols.IndexOf(col);
                    col.Width = dynamicColWidth + (idx < remainder ? 1 : 0);
                }
                col.AutoSizeMode = DataGridViewAutoSizeColumnMode.None;
                col.Resizable = DataGridViewTriState.True;
                if (col.Name != "Action")
        ...
        private void RegisterSaleMenuItem_Click(object sender, EventArgs e)
        {
            ...
            string customerEmail = ""; // Removed CustomerEmail
        ...
        private void DataGridViewPlots_SelectionChanged(object? sender, EventArgs e)
        {
            if (dataGridViewPlots.CurrentRow != null)
            {
                ...
                if (idCell?.Value != null && int.TryParse(idCell.Value.ToString(), out int plotId))
                {
                    // You can add logic here if you want to do something when a plot is selected
                }
            }
        }
        ...
        {
            var colName = dataGridViewPlots.Columns[e.ColumnIndex].Name;
            if (colName == "AmountBalance" || colName == "SalePrice" || colName == "AmountPaid" || colName == "Area")
            {
        ...
        {
            var colName = dataGridViewProperties.Columns[e.ColumnIndex].Name;
            if (colName == "AmountBalance" || colName == "BuyPrice" || colName == "AmountPaid" || colName == "TotalLoanPrinciple" || colName == "Area")
            {
                if (e.Value == null || e.Value == DBNull.Value)
                {
                    e.Value = "NILL";
        ...
        private new static void ApplyGridStyle(DataGridView grid)
        {
